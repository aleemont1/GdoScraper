...
class ExtractedOffer(BaseModel):
    ...
    name: str = Field(
        description="Name of the product and optional short description in Italian (e.g., 'Biscotti Frollini')"
    )
    brand: Optional[str] = Field(
        None,
        description="Brand name of the product if clearly indicated (e.g., 'Mulino Bianco')",
    )
    weight_or_volume: Optional[str] = Field(
        None, description="Weight, volume, or quantity (e.g., '500g', '6x1.5L')"
    )
    price: float = Field(
        description="Active promotional price in euros as decimal (e.g., 1.89)"
    )
    original_price: Optional[float] = Field(
        None, description="Original pre-discount price if indicated (e.g., 2.49)"
    )
    discount_percentage: Optional[int] = Field(
        None, description="Discount percentage value if indicated (e.g., 30)"
    )
    category: Optional[str] = Field(
        None,
        description="Standard product category in Italian (e.g., 'Alimentari', 'Bevande')",
    )
    bbox: List[int] = Field(
        description="Bounding box coordinates [ymin, xmin, ymax, xmax] normalized from 0 to 1000 framing strictly the product packaging or photo itself, excluding text description blocks, price tags, background clutter, or other products."
    )
...
def _parse_single_flyer_worker(
    driver_class, file_path: str, store_id: str
) -> List[ProductOffer]:
    """
...
class AbstractPdfFlyerDriver(AbstractSupermarketDriver):
    ...
    def fetch_promotions(self, store_id: str) -> Any:
        ...
        if not is_pdf and not is_special:
            logger.info(
                f"Checking for dynamic flyer downloads for store reference: '{store_id}'..."
            )
            downloaded_paths = self.download_flyers(store_id)
            if downloaded_paths:
                logger.info(
                    f"REST Downloader retrieved {len(downloaded_paths)} flyers."
                )
                return downloaded_paths
            else:
                logger.warning(
                    "No flyers retrieved via REST. Trying filesystem scan as fallback..."
                )
        ...
        if store_id.lower() in ("all", "downloads"):
            ...
            logger.info(
                f"Scanning downloads folder '{downloads_dir}'. Found {len(pdf_paths)} PDF flyers to scrape."
            )
            ...
            if not pdf_paths:
                logger.error(
                    f"No PDF flyer matching store ID: '{store_id}' found in '{downloads_dir}'."
                )
    ...
    def parse_promotions(self, raw_data: Any, store_id: str) -> List[ProductOffer]:
        ...
        if not isinstance(raw_data, list):
            logger.error(
                "Invalid raw data structure provided. Expected a list of file paths."
            )
            return []
        ...
        if (
            not active_store_id
            or active_store_id.endswith(".pdf")
            or "/" in active_store_id
            or "\\" in active_store_id
        ):
        ...
        logger.info(
            f"Using store ID: '{active_store_id}' for database and image tagging."
        )
        ...
        if self.parallel and len(raw_data) > 1:
            ...
            logger.info(
                f"Initiating MULTIPROCESS PARALLEL parsing on {max_workers} processes (one process per flyer)..."
            )
            ...
            with ProcessPoolExecutor(max_workers=max_workers) as executor:
                ...
                for idx, future in enumerate(futures):
                    try:
                        ...
                        logger.info(
                            f"Parallel Worker #{idx + 1} finished. Extracted {len(offers)} offers."
                        )
                    except Exception as err:
                        logger.error(
                            f"Parallel worker #{idx + 1} failed with exception: {err}",
                            exc_info=True,
                        )
        else:
        ...
        logger.info(
            f"ETL Scrape completed. Extracted a total of {len(all_parsed_offers)} {self._supermarket_name} products."
        )
    ...
    def _parse_single_flyer_file(
        ...
        logger.info(
            f"Beginning spatial ETL pipeline on flyer: {os.path.basename(file_path)}"
        )
        ...
        try:
            with pdfplumber.open(file_path) as pdf:
                ...
                if has_vector_text:
                    logger.info(
                        "Vector-based PDF circular detected. Engaging high-accuracy text grid segmenter..."
                    )
                    ...
                    if total_pages > 0:
                        ...
                        if first_page_words:
                            ...
                            validity_string = self._parser.parse_flyer_validity(
                                first_page_text
                            )
                            if validity_string:
                                logger.info(
                                    f"Flyer validity successfully resolved: '{validity_string}'"
                                )
                    ...
                    for page_idx in range(total_pages):
                        ...
                        for cell in cells:
                            try:
                                offer = self._parser.parse_cell(
                                    cell["text"], store_id, validity_string
                                )
                                if offer:
                                    # Instantiate card rendering on-demand
                                    if rendered_page_img is None:
                                        try:
                                            rendered_page_img = page.to_image(
                                                resolution=120
                                            ).original
                                        except Exception as render_err:
                                            logger.debug(
                                                f"Failed to render page image: {render_err}"
                                            )
                                            rendered_page_img = False

                                    if (
                                        rendered_page_img
                                        and rendered_page_img is not False
                                    ):
                                        offer.image_url = (
                                            self._crop_and_save_card_image_from_cached(
                                                rendered_page_img,
                                                cell["bbox"],
                                                page,
                                                store_id,
                                ...
                                cell_text = cell["text"]
                                if any(
                                    kw in cell_text.lower()
                                    for kw in ["€", "pezzo", "pezzi", "anziché"]
                                ):
                                ...
                                logger.debug(
                                    f"Cell parsing unexpected exception: {parse_err}"
                                )
                        ...
                        if self.use_claude and os.environ.get("ANTHROPIC_API_KEY"):
                            if rendered_page_img is None or rendered_page_img is False:
                                try:
                                    ...
                                    logger.error(
                                        f"Failed to render page image for Claude visual audit: {render_err}"
                                    )
                                    rendered_page_img = False
                    ...
                    logger.info(
                        f"Finished parsing vector flyer {os.path.basename(file_path)}. Extracted {flyer_offers_count} products."
                    )

                # --- STRATEGY B: Scanned PDF / Image-Only Brochure Parsing ---
                else:
                    logger.info(
                        "Scanned/Flat-image brochure detected. Engaging OCR visual extraction fallback..."
                    )
                    ...
                    if self.use_gemini:
                        ...
                        for page_idx in range(total_pages):
                            page = pdf.pages[page_idx]
                            logger.info(
                                f"Processing Page {page_idx + 1}/{total_pages} via Gemini..."
                            )
                            ...
                            try:
                                ...
                                logger.error(
                                    f"Failed to render page {page_idx + 1}: {render_err}"
                                )
                                continue
                            ...
                            try:
                                ...
                                logger.info(
                                    f"Page {page_idx + 1}: Extracted {len(offers)} offers from Gemini."
                                )
                                ...
                                for idx, o in enumerate(offers):
                                    ...
                                    image_url = self._extract_and_save_product_image(
                                        name,
                                        bbox,
                                        pil_img,
                                        store_id,
                                ...
                                logger.error(
                                    f"Gemini page parsing failed: {page_api_err}"
                                )
                                continue

                    # Engine B.2: Claude Haiku 4.5 Multimodal Visual OCR
                    elif self.use_claude:
                        parsed_offers = self._parse_scanned_flyer_via_claude(
                            file_path, store_id
                        )
                    ...
                    # Engine B.3: Local Offline Tesseract OCR (Default Fallback)
                        ...
                        if total_pages > 0:
                            ...
                            try:
                                ...
                                tokens = [
                                    t for t in ocr_data["text"] if t and t.strip()
                                ]
                                ...
                                if validity_string:
                                    logger.info(
                                        f"Flyer validity resolved offline: '{validity_string}'"
                                    )
                            except Exception as e:
                                logger.debug(
                                    f"Offline validity date extraction error: {e}"
                                )

                        for page_idx in range(total_pages):
                            page = pdf.pages[page_idx]
                            logger.info(
                                f"Segmenting Page {page_idx + 1}/{total_pages} offline..."
                            )
                            ...
                            try:
                                ...
                                for cell in cells:
                                    try:
                                        ...
                                        logger.debug(
                                            f"Cell parsing ValueError: {parse_err}"
                                        )
                                    except Exception as parse_err:
                                        logger.debug(
                                            f"Cell parsing unexpected exception: {parse_err}"
                                ...
                                logger.error(
                                    f"Failed to segment page {page_idx + 1} offline: {ocr_page_err}"
                                )

        except Exception as e:
            logger.error(
                f"Critical failure while reading PDF {os.path.basename(file_path)}: {e}"
            )

        if total_pages > 0:
            yield_per_page = len(parsed_offers) / max(1, total_pages)
            is_claude_already = self.engine == "CLAUDE" or (
                has_vector_text is False
            ...
            if (
                len(parsed_offers) < 3 or yield_per_page < 0.6
            ) and not is_claude_already:
                logger.warning(
                ...
                claude_offers = self._parse_scanned_flyer_via_claude(
                    file_path, store_id
                )
                if claude_offers:
                    logger.info(
                        f"Self-healing complete. Claude API successfully extracted {len(claude_offers)} offers."
                    )
    ...
    def _log_missed_product(
        self, file_path: str, page_idx: int, reason: str, text: str
    ) -> None:
        ...
        try:
            with open(log_file, "a", encoding="utf-8") as f:
                f.write(
                    f"[{timestamp}] FILE: {filename} | PAGE: {page_idx + 1} | REASON: {reason}\n"
                )
            ...
            if page and page_w and page_h:
                ...
                for img in getattr(page, "images", []):
                    ...
                    if x_right > x_left and y_bottom > y_top:
                        ...
                        img_cx = (img_x0 + img_x1) / 2.0
                        img_cy = (img_y0 + img_y1) / 2.0
                        center_match = (bbox_x0 - 20) <= img_cx <= (bbox_x1 + 20) and (
                            bbox_y0 - 20
                        ) <= img_cy <= (bbox_y1 + 20)
                ...
                if matching_images:
                    ...
                    logger.info(
                        f"Snapping VLM bbox for '{name}' to native PDF raster image coordinate box: x0={best_img['x0']:.1f}, top={best_img['top']:.1f}"
                    )
    ...
    def _parse_scanned_flyer_via_claude(
        self, file_path: str, store_id: str
    ) -> List[ProductOffer]:
        ...
        if not os.environ.get("ANTHROPIC_API_KEY"):
            logger.warning(
                "Cannot run Claude self-healing fallback because ANTHROPIC_API_KEY is not configured."
            )
            return []

        logger.info(
            "Engaging Anthropic Claude Sonnet API for scanned flyer visual parsing fallback..."
        )
        ...
        try:
            with pdfplumber.open(file_path) as pdf:
                ...
                for page_idx in range(total_pages):
                    page = pdf.pages[page_idx]
                    logger.info(
                        f"[Self-Healing] Processing Page {page_idx + 1}/{total_pages} via Claude..."
                    )
                    ...
                    try:
                        ...
                        logger.error(
                            f"Failed to render page {page_idx + 1} during fallback: {render_err}"
                        )
                        continue
                    ...
                    try:
                        ...
                        logger.info(
                            f"[Self-Healing] Page {page_idx + 1}: Extracted {len(offers)} offers from Claude."
                        )
                        ...
                        for o in offers:
                            ...
                            image_url = self._extract_and_save_product_image(
                                name, bbox, pil_img, store_id, offer_id, page=page
                            )
                        ...
                        logger.error(
                            f"[Self-Healing] Claude page parsing failed: {page_api_err}"
                        )
                        continue
        except Exception as e:
    ...
    def _audit_and_self_heal_page_via_claude(
        ...
        try:
            ...
            for attempt in range(max_retries):
                try:
                    ...
                    if attempt == max_retries - 1:
                        raise api_err
                    logger.warning(
                        f"Claude API failed (attempt {attempt + 1}/{max_retries}): {api_err}. Retrying..."
                    )
                    time.sleep(retry_backoff)
                    retry_backoff *= 2
            ...
            if response:
                ...
                logger.info(
                    f"[Visual Audit] Page {page_idx + 1}: Claude verified {len(offers_data)} offers."
                )

                initial_offers_by_id = {
                    init_o.offer_id: init_o for init_o in initial_offers
                }
                ...
                for o in offers_data:
                    ...
                    offer_id = hashlib.sha256(payload_str.encode("utf-8")).hexdigest()[
                        :32
                    ]

                    original_id = o.get("original_id")
                    matched_initial_offer = (
                        initial_offers_by_id.get(original_id) if original_id else None
                    )
                    ...
                    if not matched_initial_offer:
                        ...
                        for init_o in initial_offers:
                            if (
                                init_o.name.lower() == name.lower()
                                and init_o.price == price
                            ):
                    ...
                    if matched_initial_offer and matched_initial_offer.image_url:
                        ...
                        logger.info(
                            f"[Visual Audit] Reusing high-quality vector-extracted image for: '{name}'"
                        )
                    else:
            ...
            logger.error(
                f"[Visual Audit] Claude verification failed for page {page_idx + 1}: {err}"
            )
            return initial_offers
