...
class BasePdfLayoutSegmenter:
    ...
    def __init__(
        ...
        self.price_indicators = price_indicators or ["€", "pezzo", "pezzi", "anziché", "anzichè"]
    ...
    def _is_price_block(self, text: str) -> bool:
        ...
        """
        return any(kw in text.lower() for kw in self.price_indicators) or bool(self._price_decimal.search(text))
    ...
    def segment_page(self, page: Any) -> List[Dict[str, Any]]:
        ...
        logger.debug(f"Segmenting page of size {width:.1f} x {height:.1f} with {len(words)} words.")
        ...
        middle_words = [w for w in words if 100 <= (w["top"] + w["bottom"]) / 2.0 <= height - 80]
        if not middle_words:
            middle_words = words
        ...
        for col_idx, (col_x0, col_x1) in enumerate(col_ranges):
            ...
            for b_y0, b_y1 in blocks:
                b_words = [w for w in col_words if b_y0 <= (w["top"] + w["bottom"]) / 2.0 <= b_y1]
                if not b_words:
                    continue
                    
                sorted_words = sorted(b_words, key=lambda w: (round(w["top"]/3)*3, w["x0"]))
            ...
            if pending_desc:
                cells.append({
                    "bbox": (col_x0, pending_desc["y0"], col_x1, pending_desc["y1"]),
