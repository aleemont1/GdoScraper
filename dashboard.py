    ...
    def do_GET(self) -> None:
        ...
        # 3. Route: Search supermarket store locations
            ...
            if not supermarket or not q:
                self.wfile.write(json.dumps({"error": "Missing supermarket or search query parameter 'q'"}).encode("utf-8"))
                return
                
            try:
                driver = get_driver_by_name(supermarket, radius=radius)
                if not driver:
                    self.wfile.write(json.dumps({"error": f"Unknown supermarket: {supermarket}"}).encode("utf-8"))
                    return
                stores_list = driver.discover_stores(q)
                self.wfile.write(json.dumps(stores_list).encode("utf-8"))
            except Exception as e:
                self.wfile.write(json.dumps({"error": f"Failed to search store locations: {e}"}).encode("utf-8"))
            return
        ...
        # 4. Route: Fetch active flyers list for store
            ...
            if not supermarket or not store_id:
                self.wfile.write(json.dumps({"error": "Missing supermarket or store_id parameter"}).encode("utf-8"))
                return
            ...
            try:
                ...
                self.wfile.write(json.dumps({"error": f"Failed to retrieve flyers list: {e}"}).encode("utf-8"))
            return
        ...
        # 5. Route: Get active scrape logs
            ...
            if task:
                ...
                if os.path.exists(log_path):
                    ...
                    self.wfile.write(json.dumps({"error": f"No logs found for session: {task_id}"}).encode("utf-8"))
                    return
        ...
        # 7. Route: Serve dashboard static assets securely (preventing traversal attacks)
        elif self.path in ("/", "/index.html", "/dashboard") or self.path.startswith("/dashboard/"):
            ...
            if (requested_path_abs == dashboard_dir_abs or requested_path_abs.startswith(dashboard_dir_abs + os.sep)) and os.path.exists(requested_path_abs) and os.path.isfile(requested_path_abs):
                self.send_response(200)
    ...
    def do_POST(self) -> None:
        ...
        if self.path == "/api/upload":
            ...
            try:
                ...
                if not pdf_content:
                    ...
                    self.send_header("Access-Control-Allow-Origin", "*")
                    self.end_headers()
                    self.wfile.write(json.dumps({"error": "No PDF flyer content uploaded"}).encode("utf-8"))
                    return
        ...
        # 2. Route: Start dynamic automated scraping task
            ...
            try:
                ...
                if not supermarket or not store_id:
                    ...
                    self.send_header("Content-type", "application/json")
                    self.end_headers()
                    self.wfile.write(json.dumps({"error": "Missing supermarket or store_id"}).encode("utf-8"))
                    return
                ...
                if supermarket == "coop" and store_id.isdigit():
                    ...
                    resolved_code, _ = driver._resolve_store_details_by_db_id(int(store_id))
                    if resolved_code:
                ...
                self.send_header("Content-type", "application/json")
                self.end_headers()
                self.wfile.write(json.dumps({"error": f"Failed to spawn scraper subprocess: {e}"}).encode("utf-8"))
            return
        ...
        # 3. Route: Update an offer in the database
            ...
            try:
                ...
                if not supermarket or not store_id or not offer_id or not name or price_val is None:
                    self.send_response(400)
                    self.send_header("Content-type", "application/json")
                    self.end_headers()
                    self.wfile.write(json.dumps({"error": "Missing required fields for update (supermarket, store_id, offer_id, name, price)"}).encode("utf-8"))
                    return
                
                try:
                    price = float(price_val)
                    original_price = float(original_price_val) if original_price_val is not None and str(original_price_val).strip() != "" else None
                    discount_percentage = int(discount_percentage_val) if discount_percentage_val is not None and str(discount_percentage_val).strip() != "" else None
                except ValueError as e:
                    self.send_response(400)
                    self.send_header("Content-type", "application/json")
                    self.end_headers()
                    self.wfile.write(json.dumps({"error": f"Invalid numeric value formats: {e}"}).encode("utf-8"))
                    return
                ...
                self.send_header("Content-type", "application/json")
                self.end_headers()
                self.wfile.write(json.dumps({"error": f"Failed to update database record: {e}"}).encode("utf-8"))
            return
        ...
        # 4. Route: Delete an offer from the database
            ...
            try:
                ...
                if not supermarket or not store_id or not offer_id:
                    ...
                    self.send_header("Content-type", "application/json")
                    self.end_headers()
                    self.wfile.write(json.dumps({"error": "Missing required fields for deletion (supermarket, store_id, offer_id)"}).encode("utf-8"))
                    return
                ...
                self.send_header("Content-type", "application/json")
                self.end_headers()
                self.wfile.write(json.dumps({"error": f"Failed to delete database record: {e}"}).encode("utf-8"))
            return
        ...
        # 5. Route: Change product image in the database
            ...
            try:
                ...
                if not supermarket or not store_id or not offer_id or not image_content:
                    ...
                    self.send_header("Access-Control-Allow-Origin", "*")
                    self.end_headers()
                    self.wfile.write(json.dumps({"error": "Missing required fields (supermarket, store_id, offer_id, file)"}).encode("utf-8"))
                    return
                ...
                if ext not in (".png", ".jpg", ".jpeg", ".webp"):
                    ...
                    self.send_header("Access-Control-Allow-Origin", "*")
                    self.end_headers()
                    self.wfile.write(json.dumps({"error": "Unsupported image format. Only PNG, JPG, JPEG, and WEBP are supported."}).encode("utf-8"))
                    return
                ...
                safe_supermarket = "".join(c for c in supermarket if c.isalnum() or c in ("-", "_")).strip()
                safe_store_id = "".join(c for c in store_id if c.isalnum() or c in ("-", "_")).strip()
                safe_offer_id = "".join(c for c in offer_id if c.isalnum() or c in ("-", "_")).strip()
                ...
                from storage.database import get_storage
                storage = get_storage(DB_PATH)
                success = storage.update_offer(supermarket, store_id, offer_id, {"image_url": db_image_url})
                
                self.send_response(200)
                self.send_header("Content-type", "application/json")
                self.send_header("Access-Control-Allow-Origin", "*")
                self.end_headers()
                self.wfile.write(json.dumps({"success": success, "image_url": db_image_url}).encode("utf-8"))
                
            except Exception as e:
                self.send_response(500)
                self.send_header("Content-type", "application/json")
                self.end_headers()
                self.wfile.write(json.dumps({"error": f"Failed to change product image: {e}"}).encode("utf-8"))
            return
