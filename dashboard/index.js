    ...
    pageSlice.forEach(offer => {
        ...
        if (isEditing) {
            ...
            tr.innerHTML = `
                ...
                <td style="text-align: center;">
                    <div style="display: flex; justify-content: center; gap: 6px;">
                        <button class="btn btn-primary btn-action-sm" onclick="saveEdit('${offer.supermarket}', '${offer.store_id}', '${offer.offer_id}')" title="Confirm">
                            <svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round" style="vertical-align: middle;"><polyline points="20 6 9 17 4 12"></polyline></svg>
                        </button>
                        <button class="btn btn-secondary btn-action-sm" onclick="cancelEdit()" title="Cancel">
                            <svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round" style="vertical-align: middle;"><line x1="18" y1="6" x2="6" y2="18"></line><line x1="6" y1="6" x2="18" y2="18"></line></svg>
                        </button>
                    </div>
                </td>
            `;
            ...
            tr.innerHTML = `
                ...
                <td style="text-align: center;">
                    <div style="display: flex; justify-content: center; gap: 6px;">
                        <button class="btn btn-secondary btn-action-sm" onclick="startEdit('${offer.supermarket}', '${offer.store_id}', '${offer.offer_id}')" title="Edit">
                        ...
                        <button class="btn btn-danger btn-action-sm" onclick="deleteOffer('${offer.supermarket}', '${offer.store_id}', '${offer.offer_id}')" title="Delete">
...
async function selectStoreItem(storeId, itemIndex) {
    ...
    try {
        ...
        flyers.forEach(flyer => {
            ...
            cb.type = 'checkbox';
            cb.value = flyer.id;
            cb.checked = false; // Unselected by default
...
async function triggerScrapeExecution() {
    ...
    if (currentSupermarket === 'manual') {
        ...
        if ((currentSupermarket === 'coop' || currentSupermarket === 'conad') && selectedFlyerIds.length === 0) {
            showAlert("Please select at least one flyer catalog to scrape!", "warning");
            return;
        }
