...
class DpiuSupermarketDriver(AbstractApiSupermarketDriver):
    ...
    def __init__(
        ...
        self._session.headers.update(
            {"Origin": "https://dpiu.maxidi.it", "Referer": "https://dpiu.maxidi.it/"}
        )
    ...
    def _get_authorization_data(self) -> str:
        ...
        try:
            ...
            if not meta:
                raise ValueError(
                    "Could not find meta tag _authorization_data in Dpiù website HTML"
                )
    ...
    def _get_bearer_token(self) -> str:
        ...
        try:
            ...
            basic_auth_val = base64.b64encode(credential.encode("utf-8")).decode(
                "utf-8"
            )
    ...
    def discover_stores(self, store_id: str) -> List[Dict[str, Any]]:
        ...
        if lat is not None and lon is not None:
            ...
            return [
                ...
                if s.get("distance", float("inf")) <= self.radius
            ]
        else:
            matching = []
            for s in stores:
                ...
                if (
                    store_id.lower() in alias.lower()
                    or store_id.lower() in name.lower()
                    or store_id.lower() in address.lower()
                    or store_id.lower() in city.lower()
                ):
                    matching.append(
                        {
                            "id": alias,
    ...
    def fetch_promotions(self, store_id: str) -> Any:
        ...
        try:
            ...
            for p in active_promos:
                ...
                logger.info(
                    f"Scraping products for flyer promotion: '{p.get('description')}' (Alias: {promo_alias})"
                )
                ...
                while True:
                    ...
                    logger.info(
                        f"Retrieved {len(elements)} products from Page {page + 1}."
                    )
            ...
            logger.info(
                f"ETL Extraction complete. Discovered {len(all_products_raw)} raw Dpiù product elements."
            )
    ...
    def parse_promotions(self, raw_data: Any, store_id: str) -> List[ProductOffer]:
        ...
        if not raw_data or not isinstance(raw_data, dict):
            logger.error(
                "Invalid raw data structure provided. Expected a dictionary containing store_id and products list."
            )
            return []
        ...
        logger.info(
            f"Successfully normalized and validated {len(parsed_offers)} out of {len(products_list)} Dpiù product offers."
        )
        return parsed_offers

    def _parse_property(
        self, properties: List[Dict[str, Any]], code: str
    ) -> Optional[Any]:
    ...
    def _parse_single_product(
        ...
        if brand:
            name_str = re.sub(
                r"\b" + re.escape(brand) + r"\b", "", name_str, flags=re.IGNORECASE
            )
        if weight_str:
        ...
        if price is None:
            logger.debug(
                f"Skipping product '{name_str}' because final price is missing."
            )
            return None
        ...
        if unit_price is not None:
            m_unit = str(
                self._parse_property(properties, "MEASURE-UNIT") or "KG"
            ).lower()
        ...
        if start_date_raw and end_date_raw:
            try:
                ...
                validity_string = (
                    f"DAL {s_dt.strftime('%d/%m/%Y')} AL {e_dt.strftime('%d/%m/%Y')}"
                )
            except Exception:
                pass
        ...
        unique_payload = (
            f"DPIU:{store_id}:{validity_string or 'ALL'}:{name_str}:{price:.2f}"
        )
