...
def main() -> None:
    ...
    if args.supermarket == "coop":
        selected_flyer_ids = args.selected_flyer_ids.split(",") if args.selected_flyer_ids else None
        ...
        selected_flyer_slugs = args.selected_flyer_slugs.split(",") if args.selected_flyer_slugs else None
    ...
    if args.choose_store:
        if not sys.stdin.isatty():
            logger.warning("Non-interactive terminal detected. Bypassing interactive store selection.")
            ...
            if not stores:
                ...
                selected_store = prompt_selection(
                    ...
                    display_func=lambda s: (
                        ...
                        f"[{s.get('distance'):.2f} km/m]" if s.get("distance") is not None 
    ...
    if args.choose_flyer:
        ...
        if not flyers:
            logger.warning(f"No flyer catalogs discovered for store ID: {resolved_store_id}")
        elif len(flyers) == 1:
            ...
            logger.warning("Non-interactive terminal detected. Defaulting to first flyer to prevent heavy scraping.")
            selected = [flyers[0]["id"]]
            if args.supermarket == "coop":
                driver.selected_flyer_ids = selected
            elif args.supermarket == "conad":
            ...
            for idx, flyer in enumerate(flyers):
                ...
                print(f"  {idx+1}) {flyer.get('title')}{featured_str} [Validity: {flyer.get('validity')}]")
            
            while True:
                try:
                    user_input = input("Select flyer(s) to scrape (comma-separated indices, e.g. 1,3 or 'all', default: all): ").strip()
    ...
    logger.info(f"Running scraper strategy for: {args.supermarket.upper()} (Store: {resolved_store_id})")
    try:
