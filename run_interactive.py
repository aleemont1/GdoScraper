...
def scrape_coop_menu():
    ...
    if mode == "2":
        store_id = input(
            f"Enter GPS Coordinates [default: {CYAN}44.1396438,12.2464292{RESET} (Cesena)]: "
        ).strip()
        if not store_id:
    ...
    cmd = [PYTHON_EXE, "main.py", "--supermarket", "coop", "--store-id", store_id]

    # Ask if coordinates/city mode was selected
    if mode in ("", "1", "2"):
        radius = input(
            f"Enter Search Radius in km [default: {CYAN}15{RESET}]: "
        ).strip()
        if not radius:
            radius = "15"
        ...
        choose_store = (
            input("Enable Interactive Store List Selector? [y/N]: ").strip().lower()
        )
        if choose_store == "y":
            cmd.append("--choose-store")

    choose_flyer = input("Enable Interactive Flyer Selector? [y/N]: ").strip().lower()
    if choose_flyer == "y":
        cmd.append("--choose-flyer")

    db_path = input(
        f"Enter SQLite DB path [default: {CYAN}storage/promotions.db{RESET}]: "
    ).strip()
    if db_path:
...
def scrape_conad_menu():
    ...
    if mode == "2":
        store_id = input(
            f"Enter Conad Store ID (anacanId, e.g. {CYAN}005635{RESET}): "
        ).strip()
        if not store_id:
        ...
        coords = input(
            f"Enter Coordinates [default: {CYAN}44.1396438,12.2464292{RESET} (Cesena)]: "
        ).strip()
        if not coords:
    ...
    parallel = (
        input("Enable multi-process parallel flyer parsing? [y/N]: ").strip().lower()
    )
    if parallel == "y":
        cmd.append("--parallel")
    ...
    # Optional limits
    max_flyers = input(
        "Limit max flyer downloads? [Enter for no limit, or integer]: "
    ).strip()
...
def scrape_ins_menu():
    ...
    if mode == "2":
        store_id = input(
            f"Enter IN's Store Code [default: {CYAN}E-Campagna-OF{RESET}]: "
        ).strip()
        if not store_id:
    ...
    cmd = [PYTHON_EXE, "main.py", "--supermarket", "ins", "--store-id", store_id]
...
def scrape_dpiu_menu():
    ...
    if mode == "2":
        ...
        store_id = input(
            f"Enter Dpiù Store Alias [default: {CYAN}d-cesena{RESET}]: "
        ).strip()
        if not store_id:
            store_id = "d-cesena"
    else:
    ...
    cmd = [PYTHON_EXE, "main.py", "--supermarket", "dpiu", "--store-id", store_id]
...
def scrape_manual_menu():
    ...
    print(
        "Scrapes a manual PDF flyer located in downloads/uploaded/ or specified by file path."
    )
...
def display_db_stats():
    ...
    if engine != "supabase":
        ...
        if not os.path.exists(db_path):
            print(
                f"{RED}Database not found at '{db_path}'. Run a scraper first to initialize it!{RESET}"
            )
            input("\nPress Enter to return to main menu...")
            return
    ...
    try:
        ...
        print("\n" + "=" * 70)
        print(
            f"{BOLD}{'SUPERMARKET':15} | {'STORE ID':10} | {'OFFERS':10} | {'MIN PRICE':10} | {'MAX PRICE':10}{RESET}"
        )
        print("-" * 70)
        for row in breakdown:
            ...
            print(
                f"{superm:15} | {store:10} | {count:10d} | {min_pr_str:10} | {max_pr_str:10}"
            )
        print("=" * 70)
...
def dev_tools_menu():
    ...
    while True:
        ...
        print(
            f"  [{CYAN}2{RESET}] Clear Cropped Images (delete all cached product PNGs)"
        )
        print(f"  [{CYAN}3{RESET}] Clear Downloaded Conad & IN's PDF Flyer Cache")
        print(
            f"  [{RED}4{RESET}] {BOLD}FULL STORAGE WIPE{RESET} (Reset DB, Images, and PDFs)"
        )
        print("-" * 65)
        print(
            f"  [{GREEN}5{RESET}] {BOLD}Benchmark{RESET}: Run Conad Scrape (All Flyers) - {BOLD}Sequential Mode{RESET}"
        )
        print(
            f"  [{GREEN}6{RESET}] {BOLD}Benchmark{RESET}: Run Conad Scrape (All Flyers) - {BOLD}Parallel Mode{RESET}"
        )
        print(
            f"  [{GREEN}7{RESET}] {BOLD}Preset{RESET}: Run Full Cesena Scrape (Coop + Conad + IN's + Dpiù)"
        )
        print("-" * 65)
        ...
        if choice == "1":
            ...
            if engine == "supabase":
                confirm = (
                    input(
                        f"{RED}{BOLD}WARNING: Clear all records from Supabase promotions table? [y/N]: {RESET}"
                    )
                    .strip()
                    .lower()
                )
            ...
            confirm = (
                input(
                    f"{RED}{BOLD}WARNING: This will wipe ALL databases, downloaded PDFs, and cropped images! Proceed? [y/N]: {RESET}"
                )
                .strip()
                .lower()
            )
            ...
            subprocess.run(
                [PYTHON_EXE, "main.py", "--supermarket", "coop", "--store-id", "0315"]
            )
...
def shutdown_all_processes(silent=False):
    ...
    if not silent:
        if scrapers_killed > 0:
            print(
                f"{GREEN}Terminated {scrapers_killed} running scraper processes.{RESET}"
            )
        else:
            print("No running scrapers found.")

    dashboards_killed = kill_processes("dashboard.py")
    if not silent:
        if dashboards_killed > 0:
            print(
                f"{GREEN}Terminated {dashboards_killed} running dashboard server processes.{RESET}"
            )
        else:
            print("No running dashboard server found.")
...
def main_menu():
    ...
    while True:
        ...
        print(
            f"  [{GREEN}2{RESET}] {BOLD}CONAD{RESET}: Scrape PDF flyers (REST Discovery/Download/Parallel)"
        )
        print(
            f"  [{GREEN}3{RESET}] {BOLD}IN'S{RESET}: Scrape PDF flyers (BeautifulSoup Crawler/Dual-Engine OCR)"
        )
        print(
            f"  [{GREEN}4{RESET}] {BOLD}DPIÙ{RESET}: Scrape API promotions (REST Dynamic OAuth2)"
        )
        print(
            f"  [{GREEN}5{RESET}] {BOLD}MANUAL{RESET}: Scrape a manually uploaded PDF flyer"
        )
        print(
            f"  [{GREEN}6{RESET}] {BOLD}DASHBOARD{RESET}: Launch visual verification server SPA"
        )
        print(f"  [{GREEN}7{RESET}] {BOLD}STATS{RESET}: Display database analytics")
        print(
            f"  [{RED}8{RESET}] {BOLD}DEV TOOLS{RESET}: Developer utilities & benchmark presets"
        )
        print(
            f"  [{RED}9{RESET}] {BOLD}SHUTDOWN{RESET}: Kill all scraper & dashboard processes"
        )
        print(
            f"  [{RED}10{RESET}] {BOLD}HARD RELOAD{RESET}: Shutdown everything and reboot the TUI"
        )
