...
def test_parse_helpers():
    ...
    assert (
        parser.de_space_uppercase("S P A G H E T T I  Barilla") == "SPAGHETTI Barilla"
    )
...
def test_fuzzy_image_reuse(temp_db_path):
    ...
    # Exact match reuse
    url1 = find_reusable_image(
        "CONAD", "Biscotti Frollini Integrali", db_path=temp_db_path
    )
    assert url1 == "/storage/images/conad_005635_ean_12345.png"

    # Fuzzy match reuse (above 88% threshold)
    url2 = find_reusable_image(
        "CONAD", "Biscotti frollini integrali.", db_path=temp_db_path
    )
...
def test_database_update_and_delete(temp_db_path):
    ...
    cursor.execute(
        "SELECT name, price, original_price, discount_percentage FROM promotions WHERE offer_id='item_999';"
    )
    row = cursor.fetchone()
    assert row is not None
...
def test_supabase_storage_operations_mocked(monkeypatch):
    ...
    mock_get.assert_called_with(
        "https://xyz.supabase.co/rest/v1/promotions?limit=1",
